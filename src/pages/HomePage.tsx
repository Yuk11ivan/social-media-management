...
function StatItem({ label, value, suffix, icon, inView }: {
  ...
  return (
    <div className="text-center p-4 sm:p-5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm">
      ...
      <div className="text-2xl sm:text-3xl font-heading font-bold text-white font-mono">{c}{suffix || ''}</div>
...
function FeatureList() {
  return (
    <div className="grid sm:grid-cols-2 gap-3 mb-8">
      {FEATURES.map((f) => (
        <div key={f.title} className="flex gap-3 p-3 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm">
...
function StepList() {
  return (
    <div className="space-y-3 mb-8">
      {STEPS.map((s) => (
        <div key={s.step} className="flex gap-3 p-3 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm items-start">
...
function UseCaseList() {
  return (
    <div className="grid sm:grid-cols-2 gap-3 mb-8">
      {USE_CASES.map((u) => (
        <div key={u.title} className="flex gap-3 p-3 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm">
          <div className="w-9 h-9 shrink-0 rounded-lg flex items-center justify-center text-gilt-300"
            style={{ background: 'linear-gradient(135deg,rgba(255,255,255,0.12),rgba(255,255,255,0.04))' }}>
            {u.icon}
          </div>
          <div>
            ...
            <p className="text-xs text-white/60 mt-0.5 leading-relaxed">{u.desc}</p>
          </div>
        </div>
      ))}
    </div>
      ...
      {PLATFORM_ORDER.map((pid) => {
        ...
        return (
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-full border border-white/15 bg-white/8 backdrop-blur-sm text-sm font-medium text-white/80 hover:text-white hover:border-gilt-400/40 hover:bg-white/12 transition-all"
          >
  ...
  const slides: HomeSlide[] = [
      ...
      actions: (
        <>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
            ...
            className="btn-gilt inline-flex items-center justify-center gap-2.5 px-8 py-3.5 rounded-xl font-bold text-base">
            <Play className="w-5 h-5" />{token ? '开始创作' : '免费开始创作'}
          </motion.button>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
            onClick={() => navigate('/platforms')}
            className="inline-flex items-center justify-center gap-2.5 px-8 py-3.5 rounded-xl font-bold text-base border border-white/20 bg-white/10 text-white backdrop-blur-md hover:bg-white/15 transition-colors">
    ...
    },
    {
      ...
      actions: (
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
          ...
          className="btn-gilt inline-flex items-center gap-2 px-8 py-3.5 rounded-xl font-bold">
    ...
    },
    {
      ...
      actions: (
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
          onClick={() => navigate('/platforms')}
          className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl font-bold border border-white/20 bg-white/10 text-white backdrop-blur-md hover:bg-white/15 transition-colors">
    ...
    },
    {
      ...
      actions: (
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
          onClick={() => navigate(token ? '/generate' : '/account')}
          className="inline-flex items-center gap-3 px-10 py-4 rounded-xl bg-white text-gilt-700 font-bold text-lg hover:bg-crystal-50 transition-colors shadow-lg">
