...
export default function PlatformsPage() {
      ...
      return (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${st.connected ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`} />
            <span className="text-sm text-secondary">
              {st.connected ? '已连接' : isBrowser ? '待登录' : '连接异常'}
            </span>
            {st.accountName && <><span className="text-muted">·</span><span className="text-sm font-medium">{st.accountName}</span></>}
          </div>
          {isBrowser && st.appId && <p className="text-xs text-muted truncate">Profile: {st.appId}</p>}
  ...
  const pageDesc = focusPlatform
    ? (focusPlatform === 'wechat' ? '填写 AppID 与 AppSecret，绑定后即可推送图文到公众号草稿箱。'
      : '通过 Chrome 登录态绑定，绑定后即可自动填入内容编辑器。')
    : '微信、微博、小红书已支持完整绑定与推送，绑定后即可一键生成并推送内容。';
    ...
    <PageTransition>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <h1 className="text-3xl font-heading font-bold text-primary mb-3">{pageTitle}</h1>
          <p className="text-sm text-secondary max-w-md mx-auto">{pageDesc}</p>
          {focusPlatform && (
            <Link to="/platforms" className="inline-block mt-4 text-xs text-emerald-600 hover:text-emerald-700 underline-offset-2 hover:underline">
              查看全部平台
            </Link>
          )}
        </motion.div>
        ...
        <div className={`grid grid-cols-1 ${focusPlatform ? 'max-w-xl mx-auto' : 'md:grid-cols-2'} gap-6`}>
          {displayOrder.map((pid, index) => {
            ...
            return (
              ...
              >
                <Card hover={isLive} className={`relative overflow-hidden ${!isLive ? 'opacity-80' : ''} ${focusPlatform === pid ? 'ring-2 ring-offset-2 shadow-md' : ''}`}>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-heading font-bold text-lg" style={{ backgroundColor: p.color }}>
                      {p.icon === 'MessageCircle' ? 'W' : p.icon === 'Heart' ? 'R' : p.icon === 'Music' ? 'D' : 'Wb'}
                    </div>
                    <div>
                      <h3 className="text-lg font-heading font-semibold text-primary">{p.name}</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        {isLive && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-medium"><CheckCircle className="w-3 h-3" /> 可用</span>}
                  ...
                  <div className="border-t border-border pt-4">
                    {isBindable ? renderBindSection(pid as 'wechat' | 'weibo' | 'xiaohongshu')
                      : <p className="text-xs text-muted">{isMock ? 'API 集成开发中，当前支持 AI 内容生成，一键推送功能即将开放。' : '该平台已纳入开发计划，敬请期待。'}</p>}
                  </div>
          ...
          <form onSubmit={handleWechatBind} className="space-y-4">
            <div>
              ...
              <input type="text" required minLength={10} value={appId} onChange={e => setAppId(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent" placeholder="wx..." />
            </div>
            <div>
              ...
              <div className="relative">
                <input type={showSecret ? 'text' : 'password'} required minLength={10} value={appSecret} onChange={e => setAppSecret(e.target.value)}
                  className="w-full px-4 py-2.5 pr-10 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent" placeholder="••••••••••••••••" />
            ...
            <div>
              <label className="block text-sm font-medium text-secondary mb-1">名称 <span className="text-muted font-normal">（选填）</span></label>
              <input type="text" value={accountName} onChange={e => setAccountName(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent" placeholder="用于标识此账号" />
            </div>
        ...
        <Modal isOpen={modal === 'weibo'} onClose={() => setModal(null)} title="绑定微博">
          <form onSubmit={handleWeiboBind} className="space-y-4">
            <div className="text-xs text-secondary bg-blue-50 border border-blue-100 rounded-lg px-3 py-2 space-y-1">
            ...
            <div>
              <label className="block text-sm font-medium text-secondary mb-1">微博昵称 / 备注 <span className="text-muted font-normal">（选填）</span></label>
              <input type="text" value={accountName} onChange={e => setAccountName(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent" placeholder="例如：官方账号" />
            </div>
            <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className="text-xs text-muted hover:text-secondary">
              {showAdvanced ? '收起高级选项' : '展开高级选项（一般不需要填）'}
            </button>
            {showAdvanced && (
              <div className="p-4 rounded-xl bg-gray-50 border border-border">
                <label className="block text-sm font-medium text-primary mb-1.5">Chrome Profile 路径</label>
                <input type="text" value={profileDir} onChange={e => setProfileDir(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent" placeholder="留空即可，系统自动创建" />
                <p className="text-xs text-muted mt-2">系统自动在 <code className="text-xs bg-gray-200 px-1 rounded">backend/weibo_profiles/your_user_id</code> 创建</p>
              </div>
            )}
            <Button type="submit" isLoading={isBinding} className="w-full">绑定</Button>
          </form>
        ...
        <Modal isOpen={modal === 'xiaohongshu'} onClose={() => setModal(null)} title="绑定小红书">
          <form onSubmit={handleXhsBind} className="space-y-4">
            <div className="text-xs text-secondary bg-rose-50 border border-rose-100 rounded-lg px-3 py-2 space-y-1">
              <p><strong>绑定步骤：</strong></p>
              <p>1. 填写备注（选填）→ 点击「绑定」</p>
              <p>2. 绑定成功后点击「打开浏览器登录」</p>
              <p>3. 在弹出的 Chrome 窗口扫码或手机验证登录小红书</p>
              <p>4. 登录一次即可长期使用</p>
              <p className="text-amber-600 mt-1">⚠️ 基于浏览器自动化，请遵守平台规范，合理控制发布频率</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-secondary mb-1">小红书昵称 / 备注 <span className="text-muted font-normal">（选填）</span></label>
              <input type="text" value={accountName} onChange={e => setAccountName(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-rose-400 focus:border-transparent" placeholder="例如：美妆号" />
            </div>
            <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className="text-xs text-muted hover:text-secondary">
              {showAdvanced ? '收起高级选项' : '展开高级选项（一般不需要填）'}
            </button>
            {showAdvanced && (
              <div className="p-4 rounded-xl bg-gray-50 border border-border">
                <label className="block text-sm font-medium text-primary mb-1.5">Chrome Profile 路径</label>
                <input type="text" value={profileDir} onChange={e => setProfileDir(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-rose-400 focus:border-transparent" placeholder="留空即可，系统自动创建" />
                <p className="text-xs text-muted mt-2">系统自动在 <code className="text-xs bg-gray-200 px-1 rounded">backend/xiaohongshu_profiles/your_user_id</code> 创建</p>
              </div>
            )}
            <Button type="submit" isLoading={isBinding} className="w-full">绑定</Button>
          </form>
