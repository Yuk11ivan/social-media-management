...
const NAV_LINKS = [
  ...
  { to: '/material', label: '素材管理' }, { to: '/push', label: '推送发布' },
        ...
        <div className="flex items-center h-16">
          ...
          <Link to="/" className="flex items-center gap-2.5 group shrink-0">
            ...
            <div className="flex-col hidden sm:flex">
              <span className="font-heading font-bold text-lg text-crystal-900 leading-tight">Altus</span>
              <span className="text-[10px] text-crystal-500 font-medium leading-tight">奥途智营</span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-4">
            {NAV_LINKS.map((link) => (
              <NavLink key={link.to} to={link.to} end={link.to === '/'}
                className={({ isActive }) => `relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${isActive ? 'text-gilt-700' : 'text-crystal-700 hover:text-crystal-900 hover:bg-crystal-100/60'}`}>
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden p-2 rounded-lg text-crystal-700 hover:bg-crystal-100 transition-colors ml-auto">{mobileOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button>
        </div>
      </div>
